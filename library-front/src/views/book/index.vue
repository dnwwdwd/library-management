...
import {addBookAPI, deleteBooksAPI, getBookPageAPI} from '@/api/book'
    ...
    <el-table ref="multiTableRef" :data="bookList" stripe border @selection-change="handleSelectionChange"
      ...
      <el-table-column label="操作" width="200px" fixed="right">
        ...
        <template #default="scope">
          ...
          <el-button @click="show_borrow_modal(scope.row)" type="primary" v-if="!isAdmin && scope.row.stock > 0">
            借书
          </el-button>
          <el-button @click="ElMessage({type: 'success', message: '该书籍已被借光，请等待他人归还后再借'})" type="warning" v-if="!isAdmin && scope.row.stock < 1">
            已被借光
          </el-button>
        </template>
      </el-table-column>
