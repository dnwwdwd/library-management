...
import {getCurrentUser, updateAPI} from '@/api/user'

// ------ data ------
const formLabelWidth = '80px'
const managerStore = useUserStore()
...
const submit = async () => {
  try {
    ...
    if (valid) {
      ...
      const { data: res } = await updateAPI(form)
      if (res.code != 0) return   // 密码错误信息会在相应拦截器中捕获并提示
      // 然后进行 消息提示，store更新，页面跳转 等操作
      managerStore.userInfo!.name = form.name
      if (form.oldPwd !== '' && form.newPwd !== '') {
        managerStore.userInfo = null;
        router.push({
          path: '/login',
        });
        ElMessage({
...
<template>
  <el-card class="my-card">
    <h1>修改个人信息</h1>
    <el-form :model="form" ref="tmpform" class="my-form">
